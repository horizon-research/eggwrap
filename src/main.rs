...
fn main() {
    ...
    .arg(
        Arg::with_name("debug")
            ...
            .takes_value(true)
            .default_value("true")
            .help("Whether to print debug lines (True/False)"),
    )
    .arg(
        Arg::with_name("extract")
            ...
            .takes_value(true)
            .default_value("both")
            .help("Extraction method (greedy, ILP, or both)"),
    )
    .get_matches();
...
fn optimize(matches: clap::ArgMatches) {
    ...
    let print_debug = matches.value_of("debug").unwrap().parse::<bool>().unwrap();
    let extract_method = matches.value_of("extract").map(String::from).unwrap();
    ...
    // Greedy extraction
    if extract_method == "both" || extract_method == "greedy" {
        ...
        println!("{:?}", best.pretty(40 as usize));   
    }

    // ILP extraction
    if extract_method == "both" || extract_method == "ILP" {
        ...
        println!("{}", lp_best.pretty(40 as usize));
    }
}
